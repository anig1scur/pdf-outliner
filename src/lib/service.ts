...
export async function processToc(
    payload:
    { images?: string[]; text?: string; apiKey: string; provider: string; doubaoConfig?: { textEndpoint: string; visionEndpoint: string } }) {
  const { images, text, apiKey, provider, doubaoConfig } = payload;
  ...
  if (currentProvider === 'qwen') {
    ...
    jsonText = await processWithDoubao(
      isTextMode ? text! : images!, apiKey, doubaoConfig, isTextMode);
  } else {
